...
if os.getenv('TESTING'):
    ...
    chats_manager = Chats(test_client=test_engine)
else:
...
def password_reset(body: dict):
    ...
    if not sql_manager.getemail(data["email"]):
        raise HTTPException(status_code=404, detail="Account not found")
...
def send_message(destination_id: str, body: dict):
    ...
    if not sql_manager.get(data["provider_id"]):
        raise HTTPException(status_code=404, detail="Provider not found")
    if not sql_manager.get(data["client_id"]):
        raise HTTPException(status_code=404, detail="Client not found")
    
    sender_id = {data["provider_id"], data["client_id"]} - {destination_id}
...
def get_chat(provider_id: str, client_id: str, limit: int, offset: int):
    if not sql_manager.get(provider_id):
        raise HTTPException(status_code=404, detail="Provider not found")
    if not sql_manager.get(client_id):
        raise HTTPException(status_code=404, detail="Client not found")
...
def search_messages(
    ...
    if provider_id is not None and not sql_manager.get(provider_id):
        raise HTTPException(status_code=404, detail="Provider not found")
    if client_id is not None and not sql_manager.get(client_id):
        raise HTTPException(status_code=404, detail="Client not found")
    ...
    messages = chats_manager.search(limit, offset, provider_id, client_id, min_date, max_date, keywords)
    if messages is None:
        raise HTTPException(status_code=404, detail="No messages found")
    return messages
